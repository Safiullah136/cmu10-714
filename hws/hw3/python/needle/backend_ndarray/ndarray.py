...
class NDArray:
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        for x, y in zip(new_shape, self.shape):
          if x != y and y != 1:
            raise AssertionError(f"{self.shape} is not broadcastable to {new_shape}")
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, sl in enumerate(slices):
          if isinstance(sl, slice):
            ...
            new_offset += self.strides[i] * sl.start
            new_strides.append(sl.step * self.strides[i])
