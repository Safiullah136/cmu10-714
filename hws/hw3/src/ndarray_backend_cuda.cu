...
__device__ size_t indexTransform(size_t idx, CudaVec shape, CudaVec strides, size_t offset) {
  size_t strided_index = offset;
  size_t prev = 1;
  for (int i = shape.size - 1; i >= 0; i--) {
    strided_index += ( (idx % (shape.data[i] * prev)) / prev) * strides.data[i];
    prev *= shape.data[i];
  }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) out[gid] = a[indexTransform(gid, shape, strides, offset)];
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[indexTransform(gid, shape, strides, offset)] = a[gid];
}
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[indexTransform(gid, shape, strides, offset)] = val;
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 grid((M + THREADS_PER_DIM - 1) / THREADS_PER_DIM, (P + THREADS_PER_DIM - 1) / THREADS_PER_DIM);
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid >= size) return;
  scalar_t max_el = a[gid * reduce_size];
  for(size_t i = 1; i < reduce_size; i++) {
    max_el = max(max_el, a[gid * reduce_size + i]);
  }
  out[gid] = max_el;
}
